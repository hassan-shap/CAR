...
def main():
    ...
    # parameters
#     mu1=np.linspace(0,0.2,30)
#     mu2=np.linspace(0.21,0.4,100)
#     mu_list=np.concatenate((mu1,mu2),axis=0)
    mu_list=np.linspace(0,0.4,50)
    lam=0.5
    mu_sc= 0.18+t - 2*lam
    Delta=0.03
    t_j=1.0
    ...
    f1= 'twosided_vs_mu_U_%.2f_phi_%.4f_mus_%.2f_D_%.2f_tj_%.2f_W_%d_L_%d_Ls_%d_Llead_%d' % (U0,phi,mu_sc,Delta,t_j,W,L,Lsc,Llead)
    print(f1)
    f1=out_dir+f1
    for i_r in range(0,Nrep):
        print(i_r)
        salt=t0+i_r
        params=dict(t_j=t_j, gs=gs, gn=gn, lam=lam, Delta=Delta, U0=U0, salt=salt,mu_sc=mu_sc, phi=phi)
